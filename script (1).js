...
function mousePressed() {
  if (stage === 2 && !videoPlaying) {
    for (let i = 0; i < clickableNodes.length; i++) {
      let d = dist(mouseX, mouseY, clickableNodes[i].x * canvasWidth / 1080, clickableNodes[i].y * canvasHeight / 720);
    ...
    if (mouseX >= 985 * canvasWidth / 1080 && mouseX <= 1065 * canvasWidth / 1080 && mouseY >= 660 * canvasHeight / 720 && mouseY <= 690 * canvasHeight / 720) {
      returnToStageOne();
    }
  }
}
...
function drawFirstCanvas() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    line(lines[i].x1 * canvasWidth / 1080, lines[i].y1 * canvasHeight / 720, lines[i].x2 * canvasWidth / 1080, lines[i].y2 * canvasHeight / 720);
  }

  for (let i = 0; i < clickableNodes.length; i++) {
    let node = clickableNodes[i];
    let d = dist(mouseX, mouseY, node.x * canvasWidth / 1080, node.y * canvasHeight / 720);
    ...
    noStroke();
    ellipse(node.x * canvasWidth / 1080, node.y * canvasHeight / 720, radius * 2 * canvasWidth / 1080, radius * 2 * canvasHeight / 720);
  }
}
...
function drawSecondCanvas() {
  ...
  let buttonY = (height + 540 * canvasHeight / 720) / 2 + 20;
  replayButton.position(width / 2 - 30 * canvasWidth / 1080, buttonY);
  backButton.position(width / 2 + 30 * canvasWidth / 1080, buttonY);
}

function drawReturnButton() {
  fill(200);
  rect(945 * canvasWidth / 1080, 640 * canvasHeight / 720, 80 * canvasWidth / 1080, 30 * canvasHeight / 720);
  fill(0);
  textSize(16 * canvasWidth / 1080);
  textAlign(CENTER, CENTER);
  text('Back', 985 * canvasWidth / 1080, 656 * canvasHeight / 720);
}
